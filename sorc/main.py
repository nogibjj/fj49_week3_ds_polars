...
# Basic stats
def mean():
    mean_duration = int(spotify.select(pl.col("duration_ms")).mean().take(1).to_pandas()["mean_duration_ms"])
    return mean_duration

def median():
    median_duration = int(spotify.select(pl.col("duration_ms")).median().take(1).to_pandas()["median_duration_ms"])
    return median_duration

def mode():
    mode_duration = int(spotify.select(pl.col("duration_ms")).mode().take(1).to_pandas()["mode_duration_ms"])
    return mode_duration

def std():
    std_duration = int(spotify.select(pl.col("duration_ms")).std().take(1).to_pandas()["std_duration_ms"])
    return std_duration
